...
export default class FightScene_2P extends Phaser.Scene {
        ...
        callback: () => {
            ...
            if (sequence[i] === "FIGHT!") {
                this.time.delayedCall(500, () => {
                    ...
                    // ✅ Ensure players face each other immediately
                    this.handleAutoFace();
    ...
    setupHealthBars(width){
        ...
        // Player names
        this.p1Name = this.add.text(40, 70, this.player1Character.toUpperCase(), {
            fontFamily: "Arial Black", fontSize: 24, color: "#00ffff", stroke: "#000", strokeThickness: 6
        }).setOrigin(0,0.5);

        this.p2Name = this.add.text(width-40, 70, this.player2Character.toUpperCase(), {
            fontFamily: "Arial Black", fontSize: 24, color: "#ff5555", stroke: "#000", strokeThickness: 6
        }).setOrigin(1,0.5);

        // Wins
        this.p1ScoreText = this.add.text(40, 20, "Wins: " + this.p1Wins, {
            fontFamily: "Arial Black", fontSize: 24, color: "#00ffff", stroke: "#000", strokeThickness: 4
        }).setOrigin(0,0.5);

        this.p2ScoreText = this.add.text(width-40, 20, "Wins: " + this.p2Wins, {
            fontFamily: "Arial Black", fontSize: 24, color: "#ff5555", stroke: "#000", strokeThickness: 4
        }).setOrigin(1,0.5);
        ...
        // Ready text
        this.readyText = this.add.text(width/2, this.cameras.main.height/2, "", {
            fontFamily: "Arial Black", fontSize: 60, color: "#ffffff", stroke: "#000", strokeThickness: 8
        }).setOrigin(0.5).setDepth(10);
    }
    ...
    if(target.health <= 0){
        ...
        // ✅ Play death animation fully
        const deathAnim = target === this.player1 ? "p1_death" : "p2_death";
        target.play(deathAnim, true);

        // Disable movement
        target.body.moves = false;
        target.setVelocity(0);

        // Wait for animation to finish before ending the round
        this.time.delayedCall(1200, () => {  // adjust 1200ms to match your death animation duration
    ...
    endFight(winner){
        ...
        if(winner === "p1"){
            this.p1Wins++;
            this.p1ScoreText.setText("Wins: " + this.p1Wins);
            text = "PLAYER 1 WINS \n ROUND!";
        } else if(winner === "p2"){
            this.p2Wins++;
            this.p2ScoreText.setText("Wins: " + this.p2Wins);
            text = "PLAYER 2 WINS \n ROUND!";
        } else {
            text = "DRAW!";
        }
