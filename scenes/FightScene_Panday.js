...
export default class FightScene_Panday extends Phaser.Scene {
    ...
    preload() {
        ...
        charSheets.forEach(sheet => {
            this.load.spritesheet(
                "player_" + sheet,
                "/assets/characters/panday/" + sheet + ".png",
    ...
    createAnimations(prefix, enemyType=null) {
        const playerFrames = { idle:4, run:6, jump:2, fall:2, attack1:6, attack2:6, special:8, hit:3, death:6 };
    ...
    setupHealthBars(width){
        ...
        this.playerName = this.add.text(40, 70, "PANDAY", {
    ...
    endFight(winner){
        ...
        let deathAnim = targetChar.anims.currentAnim;
        let frameRate = (deathAnim && deathAnim.frameRate) ? deathAnim.frameRate : 10;
        let frameCount = (deathAnim && deathAnim.frames) ? deathAnim.frames.length : 15;
        let deathAnimDuration = (frameCount / frameRate) * 1000;
        if(!deathAnimDuration || isNaN(deathAnimDuration)) deathAnimDuration = 1500;
